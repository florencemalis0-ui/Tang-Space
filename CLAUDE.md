...
TangTang（唐睿阳）的个人品牌主页。React 18 + TypeScript + Vite 6 + react-router-dom v7，部署在 GitHub Pages 项目页 `/Tang-Space/`。首页为 Three.js 3D 过程化视觉（粒子流场 + 自写 GLSL shader）。
...
5. **One Signal Rule**：Kuaishou Orange `#ff4906` 每屏最多一处。现承载为 3D 粒子信号束（shader 内）+ 导航 active 圆点；UI 文字/卡片不用橙。所有页面深色背景，禁止白底
6. **3D 过程化优先**：首页 3D 视觉用 Three.js + 自写 GLSL shader 过程化生成，**禁止导入外部 glb/gltf 模型、禁止复用他站 shader/bundle**。手法可学，资产必须原创（零版权风险）
...
- 设计决策遵循 DESIGN.md 北极星 "The Engineer's Signal"（3D 过程化，代码即艺术）
